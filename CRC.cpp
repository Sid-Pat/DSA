...
string mod2div(string dividend, string divisor)
{
    ...
    string tmp = dividend.substr(0, pick);//creating window of size
    ...
    while (pick < n) {
        if (tmp[0] == '1')
            ...
            tmp = xor1(divisor, tmp) + dividend[pick]; //Performing xor division
        else
        ...
        // Increment pick to move further , each time taking one more element down
        pick += 1;
    }
...
int main()
{
    ...
    string data = "100100";
    string key = "1101";
    cout << "Sender side : \ndata : " << data <<" and generator polynomial : "<<key<< endl;
    encodeData(data, key);
